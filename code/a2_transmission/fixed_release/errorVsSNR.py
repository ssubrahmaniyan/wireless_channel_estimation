...
for noise_level in noise_levels:
    h_mse, d_ber = transmit(100, noise_level)
    h_mses.append(h_mse)
    d_bers.append(d_ber)

plt.plot(noise_levels, h_mses, label = 'Header MSE vs noise level', marker = 'x')
plt.plot(noise_levels, d_bers, label = 'Data BERs', marker = 'o')
plt.legend()
plt.xlabel("Noise level in (1/SNR)")
plt.ylabel('Error value')
plt.title("Plot of MSE, BER for fixed doppler frequency 100Hz as a function of SNR")
plt.show()
