...
def plot_filter(psd, Fs):
    plt.figure(figsize=(12, 8))

    # Time domain (assuming you generate h from psd)
    N = len(psd)
    h = np.fft.ifft(np.sqrt(psd)).real  # Generate impulse response from PSD
    t = np.arange(N) / Fs
...
axs[1, 1].set_title('Auto-correlations (After Filtering)')
...
#    json.dump(X_filtered[:, -1].tolist(), fo)
    ...
    for i in range(K):
            if frame > 0:
                lags, correlation = compute_cross_correlation(X[i], X[i])
                cross_corr_lines[cross_corr_idx].set_data(lags, correlation)
                lags, correlation = compute_cross_correlation(X_filtered[i], X_filtered[i])
