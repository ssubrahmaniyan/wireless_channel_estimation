...
def doppler_filter_psd(signal, Fc, Fd, fs): ###check filter response
    N = len(signal)
    ...
    H = np.nan_to_num(H)  # Replace NaNs with zero
...
Fd = 0.0005  # maximum Doppler frequency
...
axs[3].set_xlim(-N//2, N//2)  # Symmetric about zero
...
def update(frame):
    ...
    # doppler filter to the output signal
    X_filtered = np.zeros_like(X)
    for k in range(K):
        X_filtered[k, :] = doppler_filter_psd(X[k, :], Fc, Fd, fs)
    ...
    for k, line_fourier in enumerate(fourier_lines):
        if frame > 0:
            fft_values = np.abs(fftshift(fft(X_filtered[k, :])))[:N]
            line_fourier.set_data(np.arange(-N//2, N//2), fft_values)
