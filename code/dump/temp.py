...
with open("channels.txt", "w") as fo:
    ...
    for _ in range(P):
        # Generate new Gaussian samples for each component
        new_sample_real = np.random.randn(K)
        new_sample_imag = np.random.randn(K)
        
        # Compute Rayleigh-distributed samples
        new_sample = np.sqrt(new_sample_real**2 + new_sample_imag**2)
