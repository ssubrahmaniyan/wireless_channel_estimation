...
header_packet_size_ratios = [1, 2, 4, 8]
header_packet_sizes = [math.floor((1/i)*packet_size) for i in header_packet_size_ratios] # The part of the total signal that is agreed upon and known
...
def plotter():
    ...
    for i in range(M):
        plt.plot(header_packet_sizes, msearr[:][i], marker='o', label=f'Channel {i + 1}')
...
for i, header_packet_size in enumerate(header_packet_sizes):
