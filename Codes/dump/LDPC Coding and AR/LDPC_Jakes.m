...
[cfgLDPCEnc,decodercfg] = generateConfigLDPC(rate,block_length,'decoderAlgo','bp');
...
for i = 1:length(EbNo_dB)
    ...
    while (frame < max_frames && current_index + 2*block_length <= length(channel_values))
        ...
        info_bits_coded = rand(cfgLDPCEnc.NumInformationBits,1) < 0.5;
        
        % Encode the bits
        codeword = ldpcEncode(info_bits_coded, cfgLDPCEnc);
        
        % BPSK modulation
        symbols_coded = 1 - 2 * codeword;
        
        % Get channel values for this frame
        channel_coded = channel_values(current_index:current_index + block_length - 1);
        current_index = current_index + block_length;
        
        % Generate complex noise
        noise_coded = sqrt(noise_var) * (randn(block_length, 1) + 1j * randn(block_length, 1));
        
        % Pass through channel with noise
        received_coded = channel_coded .* symbols_coded + noise_coded/sqrt(SNR_linear);
        
        % Compute LLRs
        llr_coded = real(2 * conj(channel_coded) .* received_coded ./ (abs(channel_coded).^2 * (noise_var)));
        
        % Decode using LDPC
        [decoded_codeword, actualnumiter, finalchecks] = ldpcDecode(llr_coded, decodercfg, max_iter);
        
        % Count bit errors for coded transmission
        bit_errors_coded = bit_errors_coded + sum(decoded_codeword(1:cfgLDPCEnc.NumInformationBits) ~= info_bits_coded);
        total_bits_coded = total_bits_coded + cfgLDPCEnc.NumInformationBits;
...
% Plot BER vs Eb/No
figure;
semilogy(EbNo_dB, BER_coded, '-o', 'LineWidth', 2, 'DisplayName', 'Coded Jakes');
hold on;
semilogy(EbNo_dB, BER_uncoded1, '-s', 'LineWidth', 2, 'DisplayName', 'Uncoded Jakes');
semilogy(EbNo_dB, BER_uncoded2, '-s', 'LineWidth', 2, 'DisplayName', 'Uncoded Rayleigh');
grid on;
xlabel('Eb/No (dB)');
